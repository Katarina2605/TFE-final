...
<div class="container mx-auto px-4 mt-20">
    <header class="mb-4">
        <h1 class="text-3xl font-bold">Demandes d'ajout de refuges</h1>
    </header>
    <!-- Affichage des refuges ajoutés -->
    @if(isset($refugeRequests) && $refugeRequests->count() > 0)
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white rounded-lg">
                <thead class="bg-gray-900 text-white">
                <tr>
                    <th class="py-2">Nom du Refuge</th>
                    <th class="py-2">Adresse du Refuge</th>
                    <th class="py-2">Numéro du Refuge</th>
                    <th class="py-2">Site Web du Refuge</th>
                </tr>
                </thead>
                <tbody class="text-gray-600">
                @foreach($refugeRequests as $refuge)
                    <tr>
                        <td class="border px-4 py-2">{{ $refuge->nom_refuge }}</td>
                        <td class="border px-4 py-2">{{ $refuge->adresse_refuge }}</td>
                        <td class="border px-4 py-2">{{ $refuge->numero_refuge }}</td>
                        <td class="border px-4 py-2">
                            @if($refuge->site_web_refuge)
                                <a href="{{ $refuge->site_web_refuge }}" class="text-blue-500" target="_blank">{{ $refuge->site_web_refuge }}</a>
                            @else
                                <span class="italic">Non spécifié</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
    ...
    @else
        <p class="mt-4 italic text-gray-500">Aucun refuge ajouté pour le moment.</p>
    @endif
    <!-- Fin de l'affichage des refuges ajoutés -->
</div>
...
<style>
    ...
    #footer {
        flex-shrink: 0; /* Ne pas rétrécir le pied de page */
        margin-top: auto; /* Maintenir le pied de page en bas */
        color: white;
        padding: 20px;
    }
</style>
